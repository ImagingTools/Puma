...
管理页面仅在当前产品 ID 的上下文中读取和写入角色与权限。如果某个用户在
多个产品中拥有权限，保存时其他产品的分配保持不变。
...
选择并妥善保管。超级用户的密码同样受[第 9.4 节](#94-密码策略)所述密码策略
的约束。
...
凭证无效、连接缺失或服务器组件缺失时的错误会被报告为登录失败。已停用的
账户只有在凭证校验通过之后才会以专门的提示被拒绝，因此无法通过登录尝试
探知账户状态（参见[第 9.5 节](#95-启用与停用账户)）。
...
1. **停用账户（推荐）：** 超级用户在用户编辑器中将账户状态设置为已停用。
   用户记录及其角色和组成员关系保持不变，但无法再登录。该账户正在使用的
   会话和 PAT 会立即被拒绝。详情参见
   [第 9.5 节](#95-启用与停用账户)。
2. **移除账户：** 客户端 SDK 提供 `RemoveUser()` 作为永久删除。角色和组
   分配会随用户一并移除。删除之前必须检查业务上的保留和审计要求。

如果只需收回单项功能，移除相应的角色或组分配即可。
...
5. 立即将 secret 保存到 Secret Store 中。它以服务器组合中配置的前缀开头，
   默认为 `imt_pat_`。
...
`GetTokenPermissions()` 现在同时接受会话令牌和 PAT。对于 PAT，服务器返回
用户权限与令牌 scope 的交集，并考虑令牌的产品绑定。用户虽通过角色拥有但
不在令牌 scope 内的权限不会被返回。账户被停用时，其 PAT 同样会被拒绝。
...
Puma 在创建用户、修改密码以及初始化超级用户时校验密码，所用的是 ImtCore
用户管理中的密码策略。它在随附的 Puma 组合中已经接入，无需额外配置即按
以下规定生效：
...
| 最小长度 | 8 个字符 |
| 最大长度 | 128 个字符 |
| 小写字母、大写字母、数字和特殊字符要求 | 均不强制 |
| 以登录名作为密码 | 拒绝 |
| 已知弱口令阻止列表 | 未设置；可选择以文件方式提供 |
| 密码历史 | 最近 5 个密码不得重复使用 |
| 密码最短使用期 | 0 天，即下次修改前无需等待 |
| 密码最长使用期 | 0 天，即不过期 |
| 过期前的提醒期 | 14 天 |

其他说明：

- 拒绝时会指出被违反的规则，例如最小长度、缺少字符类别、以登录名作为
  密码、命中阻止列表、重复使用或最短使用期。管理界面会以明文提示显示。
- 空格不计为特殊字符。
- 管理员无需他人的旧密码即可设置其密码，并且不受最短使用期限制。
- 该策略不适用于来自 Windows 域的用户；他们的密码由域管理和校验。
- 登录时的过期检查以及客户端对规则的查询在服务器端已经存在，但在随附的
  Puma 组合中尚未与登录控制器连接。只有在那里同样接入该策略后，它们才会
  生效。

### 9.5 启用与停用账户

除角色之外，每个账户还具有账户状态。已停用的账户连同其全部分配都会保留，
但无法登录。
...
*图 19：账户状态以及由超级用户触发的状态转换。*

规则：

- 只有超级用户才能在用户编辑器中看到并操作账户状态开关。其他管理员看不到
  该开关，他们对用户的修改也不会改变该状态。
- 只有超级用户才能直接创建处于停用状态的账户。自助注册始终创建启用的账户。
- 登录时首先校验凭证。之后已停用的账户才会得到一条专门的提示，说明该账户
  已被停用。密码错误仍返回通用提示，因此无法通过登录尝试判断账户状态。
- 已停用账户正在使用的会话和 PAT 会立即被拒绝，无需先注销。
- 来自较旧数据集、未保存状态的账户按启用处理。

当需要保留分配关系、凭据记录或职责归属时，停用优于删除。

### 9.6 登录失败后的锁定

ImtCore 提供了账户锁定功能，在连续多次失败尝试后会暂时拒绝该账户。其默认
值为：

| 设置 | 默认值 | 含义 |
|---|---|---|
| 最大失败次数 | 5 | 触发锁定前的连续失败次数；0 表示关闭锁定 |
| 观察期 | 300 秒 | 统计失败尝试的时间窗口 |
| 锁定时长 | 900 秒 | 拒绝持续的时间；0 表示锁定直到管理员解除 |

计数器和锁定保存在内存中，服务器重启后会被重置。管理员可以提前解除锁定。
通过 Windows 域的登录路径不计入统计。

> **提示：** 随附的 Puma 组合中未接入锁定组件，因此不会发生自动锁定。如需
> 该功能，必须在服务器组合中补充。在此之前，应通过日志监控失败的登录
> （参见[第 9.2 节](#92-定期检查)）。
...
| 登录提示账户已停用 | 账户已被停用 | 请超级用户检查账户状态（第 9.5 节） |
| 密码修改被拒绝 | 违反密码策略 | 分析所提示的规则（第 9.4 节） |
